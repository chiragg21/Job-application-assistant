...
Core methods:
    ...
    fetch_table_where()   - fetch rows with optional column/filter selection
    exists()              - check if a row exists by criteria
    upsert()              - insert or update on conflict
    insert_or_ignore()    - insert, skip on UNIQUE conflict
    fetch_by_hash()       - fast hash-based lookup (for jd_parser duplicate check)
    add_one()             - insert a row and return new id
    bulk_insert()         - insert many rows efficiently in one transaction
    update_data()         - update rows matching criteria
    execute_raw()         - run a raw SQL string
    count()               - count rows with optional filters
    get_table_schema()    - column names and types for a table
...
from sqlalchemy import (
    ...
    insert, update, select, inspect, func
)
...
class SQLHandler:
    ...
    def fetch_section_items_by_ids(self, sql_ids: list[int]) -> list[dict]:
        ...
        # Safe — values are validated int ids from ChromaDB metadata
